...
class AsyncStreamPortal
    : public portal::Portal, public std::enable_shared_from_this<AsyncStreamPortal<Protocol>> {
public:
  using Pointer = std::shared_ptr<AsyncStreamPortal>;
  using ReceiveHandler = std::function<void (Pointer, std::vector<uint8_t>)>;
  using ErrorHandler = std::function<void (Pointer, const boost::system::error_code &)>;
  ...
  static Pointer create(Socket *socket, ReceiveHandler receiveHandler,
    ...
    return Pointer(new AsyncStreamPortal(socket, receiveHandler, errorHandler));
  }
  ...
  std::shared_ptr<DataWriter> beginWrite(const std::size_t messageSize) override {
    ...
    if (isClosed_) {
      throw std::runtime_error("cannot write to portal, portal is closed");
    }

    return AsyncDataWriter::Pointer(new AsyncDataWriter(messageSize, frameWriter_));
  }
...
private:
  ...
  AsyncStreamPortal(Socket *socket, ReceiveHandler receiveHandler,
                    ErrorHandler errorHandler)
      : socket_(socket), receiveHandler_(receiveHandler),
        errorHandler_(errorHandler) {}

  void initComponents() {
    auto receiveHandler =
        std::bind(&AsyncStreamPortal::handleMessage, this->shared_from_this(),
                  std::placeholders::_1);
    auto errorHandler =
