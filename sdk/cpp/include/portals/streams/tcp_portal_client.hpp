...
class TcpPortalClient : public std::enable_shared_from_this<TcpPortalClient>, public portal::Portal {
public:
  using Pointer = std::shared_ptr<TcpPortalClient>;
  using AsyncConnectHandler = std::function<void (const boost::system::error_code &, Pointer)>;
  using Portal = AsyncStreamPortal<boost::asio::ip::tcp>;

  static Pointer connect(std::shared_ptr<network::EdgeManager> edgeManager, const std::string &host, uint_least16_t port) {
  ...
  static void connectAsync(std::shared_ptr<network::EdgeManager> edgeManager, const std::string &host, uint_least16_t port,
                              AsyncConnectHandler asyncConnectHandler) {
    Pointer portal = create(edgeManager);
    portal->performAsyncConnection(host, port, asyncConnectHandler);
  }

  std::shared_ptr<DataWriter> beginWrite(const std::size_t messageSize) override {
...
private:
  ...
  static Pointer create(std::shared_ptr<network::EdgeManager> edgeManager) { return Pointer(new TcpPortalClient(edgeManager)); }

  TcpPortalClient(std::shared_ptr<network::EdgeManager> edgeManager) : portal_(nullptr), networkEdge_(nullptr), edgeManager_(edgeManager) {}
    ...
    boost::asio::async_connect(
        *socket, endpointIterator,
        [self, asyncConnectHandler](
          ...
          if (!error) {
            self->portal_->start();
            self->networkEdge_ = self->edgeManager_->addConnectingEdge(self);
          }
